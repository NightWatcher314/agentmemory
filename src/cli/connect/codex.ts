...
export const adapter: ConnectAdapter = {
  ...
  async install(opts: ConnectOptions): Promise<ConnectResult> {
    ...
    p.log.info(
      "Codex picks up MCP servers on next launch. For the deeper plugin install, run: codex plugin marketplace add NightWatcher314/agentmemory && codex plugin install agentmemory",
    );
